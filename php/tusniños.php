...
$html = "";
$cadena = 'SELECT n.niñociesc, n.niñonom1, n.niñonom2, n.niñoapel1, n.niñoapel2, n.niñosex, n.niñofoto, n.niñofecnac FROM abniño AS n, abpar AS p WHERE n.niñociesc = p.niñociesc AND p.persci = ? GROUP BY n.niñociesc DESC';
...
$sql = $conexion->prepare($cadena);
$sql->bind_param("si", $cedula, $offset);
$sql->execute();
...
if ($n > 1) {
    ...
    while ($i <= $f) {
        ...
        if ($i == $pos) {
            $html .= '<a id="pos" onclick="Cargar_TusNiños(' . $paginador . ',' . $i . ');">' . $i . '</a>';
        } else {
            $html .= '<a onclick="Cargar_TusNiños(' . $paginador . ',' . $i . ');">' . $i . '</a>';
        };
