...
if (empty($busqueda)) {
    $sql = $database->prepare('SELECT * FROM tablniño;');
    $sql->execute();
...
$cadena = "";
$cadena .= 'SELECT n.nñcedesc, n.nñprnomb, n.nñsgnomb, n.nñprapel, n.nñsgapel, n.nñgenero, n.nñfecnac, n.nñnacion, n.nñestado, n.nñperfil ';
$cadena .= 'FROM tablniño AS n ';
$cadena .= 'WHERE n.nñcedesc LIKE "%' . $busqueda . '%" OR n.nñprnomb LIKE "%' . $busqueda . '%" OR n.nñsgnomb LIKE "%' . $busqueda . '%" OR n.nñprapel LIKE "%' . $busqueda . '%" OR n.nñsgapel LIKE "%' . $busqueda . '%" ';
$cadena .= 'ORDER BY n.nñcedesc ASC ';
...
$cadena .= 'LIMIT ' . $limite . ' OFFSET ' . $offset;
$sql = $database->prepare($cadena);
$sql->execute();
...
while ($niño = $niños->fetch_assoc()) {
    ...
    if ($bandera && $niño['nñestado'] == "Habilitado") {
    ...
    if ($niño['nñestado'] == "Habilitado") {
        $html .= '<button onclick="Alerta(1, \'' . htmlspecialchars($niño['nñcedesc']) . '\');" class="btn btn-danger">';
        ...
        $html .= '<button onclick="Alerta(2, \'' . htmlspecialchars($niño['nñcedesc']) . '\');" class="btn btn-danger">';
        $html .= '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-trash" viewBox="0 0 16 16">';
        $html .= '<path d="M5.5 5.5A.5.5 0 0 1 6 6v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5m2.5 0a.5.5 0 0 1 .5.5v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5m3 .5a.5.5 0 0 0-1 0v6a.5.5 0 0 0 1 0z" />';
        $html .= '<path d="M14.5 3a1 1 0 0 1-1 1H13v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V4h-.5a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1H6a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1h3.5a1 1 0 0 1 1 1zM4.118 4 4 4.059V13a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1V4.059L11.882 4zM2.5 3h11V2h-11z" />';
        $html .= '</svg></button>';
...
$html .= '<li class="page-item"><button class="page-link" onclick="Todos(1);">Primero</button></li>';

if ($total_nivel <= 3) {
    for ($i = 1; $i <= $total_nivel; $i++) {
        if ($i == $pos) {
            $html .= '<li class="page-item active"><button class="page-link" onclick="Todos(' . $i . ');">' . $i  . '</button></li>';
        } else {
            $html .= '<li class="page-item"><button class="page-link" onclick="Todos(' . $i  . ');">' . $i . '</button></li>';
        };
    };
} else {
...
$html .= '<li class="page-item"><button class="page-link" onclick="Todos(' . $total_nivel . ');">Ultimo</button></li>';
$html .= '</ul></nav>';
